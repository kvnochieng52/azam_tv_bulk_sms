        ...
        <div class="modal-content csv-upload-modal">
          ...
          <div class="modal-body">
              ...
              >
                <div v-if="isValidatingCsv" class="upload-loading">
                  ...
                  <div class="upload-text mt-3">
                    Validating and processing your CSV file...
                  </div>
            ...
            <div class="csv-format-info">
              ...
              <ul>
                ...
                <li>
                  Must include a column labeled 'phone', 'mobile', 'Telephonee'
                  or 'contact'
                </li>
              </ul>
            </div>
          ...
          <div class="modal-footer">
            ...
            <button
              ...
              :disabled="!csvFile || isValidatingCsv || csvValidationError"
            >
              <i
                class="fas"
                :class="
                  isValidatingCsv ? 'fa-spinner fa-spin' : 'fa-check me-1'
                "
              ></i>
              {{ isValidatingCsv ? "Processing..." : "Upload File" }}
            </button>
          </div>
        </div>
      </div>
        ...
        <div class="modal-content">
            ...
            <div v-else-if="preview" class="sms-preview-container">
              ...
              <div class="card mb-4 message-preview-card">
                <div class="card-header bg-primary text-white">
                  <h6 class="m-0">
                    <i class="fas fa-comment-alt me-2"></i>Message Preview
                  </h6>
                </div>
              ...
              <div class="card mb-4">
                <div class="card-header bg-primary text-white">
                  <h6 class="m-0">
                    <i class="fas fa-address-book me-2"></i>Contact Statistics
                  </h6>
                </div>
          ...
          <div class="modal-footer">
            ...
            <button
              ...
              @click="confirmSend"
              :disabled="
                isLoadingPreview || (preview && preview.validContacts === 0)
              "
            >
...
const remainingChars = computed(
  () => 160 - (form.message ? form.message.length : 0)
);
...
onMounted(() => {
  document.addEventListener("click", (event) => {
    ...
    if (
      showColumnDropdown.value &&
      columnSelector &&
      !columnSelector.contains(event.target)
    ) {
    ...
    if (file.type === "text/csv" || file.name.endsWith(".csv")) {
      ...
      try {
        ...
        if (response.data.status === "success") {
          ...
          csvValidationError.value =
            response.data.message || "CSV validation failed";
          csvFile.value = null;
          // Clear file input
        ...
        csvValidationError.value =
          ...
          "An error occurred during CSV validation";
  ...
  if (messageTextarea) {
    ...
    setTimeout(() => {
      ...
      messageTextarea.setSelectionRange(
        start + columnPlaceholder.length,
        start + columnPlaceholder.length
      );
    }, 0);
...
const validateForm = () => {
  ...
  if (form.contact_type === "manual") {
    if (!form.recepient_contacts || form.recepient_contacts.trim() === "") {
      errors.value.recepient_contacts =
        "Please enter comma-separated contact numbers";
      isValid = false;
    }
  }
  ...
  if (form.contact_type === "contact_list") {
    if (
      !form.contact_list ||
      !Array.isArray(form.contact_list) ||
      form.contact_list.length === 0
    ) {
  ...
  if (
    form.scheduled &&
    (!scheduleDateInput.value || !scheduleTimeInput.value)
  ) {
...
const previewMessage = async () => {
  ...
  try {
    ...
    if (response.data.status === "success" && response.data.preview) {
      ...
      console.log(
        "Personalized message received:",
        preview.value.personalizedMessage
      );
      ...
      preview.value.totalContacts =
        preview.value.totalContacts || preview.value.contacts_count || 0;
      preview.value.messageTotalChars =
        preview.value.messageTotalChars || form.message.length;
      ...
      preview.value.totalContacts =
        response.data.totalContacts || response.data.contacts_count || 0;
      preview.value.messageTotalChars =
        response.data.messageTotalChars || form.message.length;

      // Properly handle personalized message
      preview.value.personalizedMessage =
        response.data.personalizedMessage || form.message;
      preview.value.originalMessage =
        response.data.originalMessage || form.message;
      ...
      if (form.contact_type === "csv" && response.data.customizationInfo) {
        ...
        preview.value.customizationInfo =
          "Message shown is personalized using the first row of your CSV data. Each recipient will receive their own customized message.";
      }
    }
    ...
    if (error.response && error.response.data && error.response.data.errors) {
      ...
      alert(
        "An error occurred while previewing your message. Please try again."
      );
    }
  } finally {
